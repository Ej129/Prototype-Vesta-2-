...
const SocialLoginModal: React.FC<SocialLoginModalProps> = ({ provider, onLogin, onClose }) => {
    ...
    <>
      ...
      <form onSubmit={handleSubmit}>
        <input
          ...
          required
          className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 text-vesta-text dark:text-gray-100"
          autoFocus
        />
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="mt-6">
        <input
          ...
          required
          className="w-full px-3 py-2 border-b-2 border-gray-500 focus:outline-none focus:border-blue-600 bg-transparent text-vesta-text dark:text-gray-100"
          autoFocus
        />
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8 max-w-sm w-full transform transition-all animate-fade-in-up" onClick={e => e.stopPropagation()}>
