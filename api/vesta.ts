...
export async function improvePlan(planContent: string, report: AnalysisReport): Promise<string> {
    ...
    try {
        const response: GenerateContentResponse = await ai.models.generateContent({
            model: "gemini-2.5-flash",
            contents: `The following project plan has been analyzed and several issues were found. Please rewrite the entire document to incorporate the recommendations and fix the issues. Improve the overall clarity, conciseness, and structure of the plan.\n\nORIGINAL PLAN:\n---\n${planContent}\n---\n\nISSUES AND RECOMMENDATIONS:\n---\n${findingsSummary}\n---\n\nReturn only the full, revised text of the improved project plan. Do not include any introductory text like "Here is the revised plan." or any other commentary.`,
            config: {
                systemInstruction: "You are an expert technical writer and project manager. Your task is to revise a project plan to resolve issues identified in an analysis report. You must integrate the given recommendations seamlessly, improve clarity, and ensure the document is professional and well-structured.",
            },
        });
